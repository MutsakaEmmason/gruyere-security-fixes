...
def _ExpandFor(tag, template, specials, block_data):
  ...
  elif isinstance(block_data, collections.abc.Sequence) and not isinstance(block_data, str):
...
def _ExpandVariable(var, specials, params, name, default=''):
  ...
  if escaper_name == 'text':
    ...
    value = '<pre>' + html.escape(pprint.pformat(value)) + '</pre>'
...
def _GetValue(collection, index, default=''):
  ...
  elif (isinstance(collection, collections.abc.Sequence) and not isinstance(collection, str) and index.isdigit() and
