...
class GruyereRequestHandler(BaseHTTPRequestHandler):
  ...
  # FIX 2 (DoS): Added '/quitserver' so only admins can shut down the server.
  # Previously '/quitserver' was missing from this list, allowing ANY user to
  # trigger a full server shutdown.
  _PROTECTED_URLS = [
      '/quit',
      '/quitserver',
      '/reset'
  ...
  def _DoLogin(self, cookie, specials, params):
    ...
    FIX 3 (SQL Injection / Credential Exposure):
      - Credentials must now arrive via HTTP POST (enforced in login.gtl).
        GET-based login exposed passwords in URLs, browser history, and logs.
      - The uid is validated against a strict alphanumeric whitelist, blocking
        SQL metacharacters and other injection attempts.
      - Password comparison uses a constant-time check to resist timing attacks.
    ...
    if 'uid' in params and 'pw' in params:
      ...
      pw  = self._GetParameter(params, 'pw')

      # FIX 3: Validate uid format — reject anything containing SQL/HTML chars
      if not uid or not UID_PATTERN.match(uid):
        message = 'Invalid user name or password.'
      elif uid in database:
        stored_pw = database[uid].get('pw', '')
  ...
  def _DoQuitserver(self, cookie, specials, params):
    ...
    FIX 2 (DoS): Added a secondary admin check inside the handler as
    defence-in-depth. Even if the _PROTECTED_URLS check is bypassed, the
    server will not shut down unless the caller is a verified admin.
    ...
    """
    # Defence-in-depth: double-check admin privilege inside the handler
  ...
  # -------------------------------------------------------------------------
  # SECURITY FIX 1 (XSS) + FIX 4 (Buffer Overflow): Enforce maximum snippet
  # length and HTML-escape all user input before storing. This prevents stored
  # Cross-Site Scripting attacks and blocks excessively long inputs.
  # -------------------------------------------------------------------------
  ...
  def _DoNewsnippet2(self, cookie, specials, params):
    ...
    if not snippet:
      ...
      # FIX 4 (Buffer Overflow): Reject snippets that exceed the maximum length
      if len(snippet) > self.MAX_SNIPPET_LENGTH:
        self._SendError(
            'Snippet is too long (max %d characters).' % self.MAX_SNIPPET_LENGTH,
            cookie, specials, params)
        return
      # FIX 1 (XSS): Escape HTML entities so injected tags are rendered as
      # plain text rather than executed as markup or script.
  ...
  def _SendHtmlResponse(self, html, new_cookie_text=None):
    ...
    # FIX 1 (XSS): Re-enable browser XSS protection (was set to '0' which
    # disabled it). Also add a Content Security Policy to block inline scripts.
  ...
  def _CreateCookie(self, cookie_name, uid):
    ...
    h_data = str(hash(str(cookie_secret) + str(c_data)) & 0x7FFFFFF)
  ...
  def _ParseCookie(self, cookie):
    ...
    try:
      ...
      # global cookie_secret
      if hashed != str(hash(str(cookie_secret) + str(cookie_data)) & 0x7FFFFFF):
        return self.NULL_COOKIE
  ...
  def DoGetOrPost(self):
    ...
    if request_ip not in allowed_ips:                        # DO NOT CHANGE
      print('DANGER! Request from bad ip: ' + request_ip,     # DO NOT CHANGE
    ...
    if (server_unique_id not in path                         # DO NOT CHANGE
      ...
      if path == '' or path == '/':                          # DO NOT CHANGE
        ...
        print('DANGER! Request without unique id: ' + path,  # DO NOT CHANGE
              file=sys.stderr)                               # DO NOT CHANGE
  ...
  def HandleRequest(self, path, query, unique_id):
    ...
    if not path:
      ...
      return
    params = parse_qs(query)  # url.query
    specials = {}
